...
static IplImage* convert_to_gray32(IplImage* img)
{
	...
	CvSize i_size;
	i_size.height = img->height;
	i_size.width = img->width;
	gray32 = cvCreateImage(i_size, IPL_DEPTH_32F, 1);//创建32位单通道图像
	...
	{
		gray8 = cvCreateImage(i_size, IPL_DEPTH_8U, 1);//创建8位单通道图像
	...
	int intvls, double sigma)
{
	...
	for (o = 0; o < octvs; o++)//遍历组
		for (i = 0; i < intvls + 3; i++)//遍历层
		{
			...
			{   //创建图像
				CvSize pyr_size;
				pyr_size.height = gauss_pyr[o][i - 1]->height;
				pyr_size.width = gauss_pyr[o][i - 1]->width;
				gauss_pyr[o][i] = cvCreateImage(pyr_size, IPL_DEPTH_32F, 1);
...
static IplImage*** build_dog_pyr(IplImage*** gauss_pyr, int octvs, int intvls)
{
	...
	for (o = 0; o < octvs; o++)//遍历组
		...
		{   //创建DoG金字塔的第o组第i层的差分图像
			CvSize pyr_size;
			pyr_size.height = gauss_pyr[o][i]->height;
			pyr_size.width = gauss_pyr[o][i]->width;
			dog_pyr[o][i] = cvCreateImage(pyr_size, IPL_DEPTH_32F, 1);
